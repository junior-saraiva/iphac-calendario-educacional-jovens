-- Só executar se a tabela feriado existir
DO $$
BEGIN
  IF EXISTS (
    ...
    -- Renomear índices existentes
    IF EXISTS (
      SELECT 1 FROM pg_indexes 
      WHERE schemaname='public' AND indexname='idx_feriado_ano'
    ) THEN
    ...
    IF EXISTS (
      SELECT 1 FROM pg_indexes 
      WHERE schemaname='public' AND indexname='idx_feriado_uf_ibge_code'
    ) THEN
      EXECUTE 'ALTER INDEX public.idx_feriado_uf_ibge_code RENAME TO feriado_alcance_idx';
    END IF;

    -- Garantir unique index para upsert via PostgREST
    IF NOT EXISTS (
      SELECT 1 FROM pg_indexes
